...
export function RootsTouchedSkyAnimator() {
  const animation = useGameStore((state) => state.rootsTouchedSkyAnimation);
  ...
  return createPortal(
    <span key={animation.id} className="roots-touched-sky-vfx" aria-hidden="true">
      <span className="roots-touched-sky-impact-haze" />
      <svg className="roots-touched-sky-slashes" viewBox="0 0 100 140" preserveAspectRatio="none">
        <defs>
        ...
        <path
          className="roots-touched-sky-slash roots-touched-sky-slash-back"
          pathLength="1"
        ...
        />
        <path
          className="roots-touched-sky-slash roots-touched-sky-slash-front"
          pathLength="1"
      ...
      <span className="roots-touched-sky-fragments">
        {FALLING_FRAGMENTS.map((fragment, index) => (
          <i
            key={`${fragment.kind}-${index}`}
            className={`roots-touched-sky-fragment roots-touched-sky-fragment-${fragment.kind}`}
            style={{
