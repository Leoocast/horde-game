...
export function FleshRootTitheSelectionOverlay({ game: _game }: { game: GameState }) {
  const fleshRootTitheSelection = useGameStore((state) => state.fleshRootTitheSelection);
  const fleshRootTitheCard = useGameStore((state) => state.fleshRootTitheCard);
  const updatePointer = useGameStore((state) => state.updateFleshRootTitheSelectionPointer);
  const deselectTarget = useGameStore((state) => state.deselectFleshRootTitheSelectionTarget);
  const [start, setStart] = useState({ x: 0, y: 0 });
  const [lockedEnd, setLockedEnd] = useState<{ x: number; y: number } | undefined>();

  const end = lockedEnd ?? (fleshRootTitheSelection ? { x: fleshRootTitheSelection.x, y: fleshRootTitheSelection.y } : undefined);
  ...
  useEffect(() => {
    ...
    function contextMenu(event: MouseEvent) {
      ...
      if (useGameStore.getState().fleshRootTitheSelection?.targetId) deselectTarget();
    }
  ...
  }, [fleshRootTitheSelection, deselectTarget, updatePointer]);

  useLayoutEffect(() => {
    if (!fleshRootTitheSelection || !fleshRootTitheCard) return;
    const activeSelection = fleshRootTitheSelection;
    let frame = 0;

    function measure() {
      const sourceRect = document.querySelector<HTMLElement>(`[data-card-id="${fleshRootTitheCard!.instanceId}"]`)?.getBoundingClientRect();
      if (sourceRect) {
        setStart({
      ...
      if (activeSelection.targetId) {
        const targetElement = activeSelection.kind === "sacrifice-land"
          ? document.querySelector<HTMLElement>("[data-flesh-root-tithe-mana-target='true']")
  ...
  }, [fleshRootTitheSelection, fleshRootTitheCard]);

  if (!fleshRootTitheSelection || !end) return null;
  ...
  return (
    <>
      ...
      <svg className="pointer-events-none fixed inset-0 z-[111] h-screen w-screen overflow-visible">
        <defs>
          <filter id="flesh-root-tithe-target-arrow-glow" x="-80%" y="-80%" width="260%" height="260%" colorInterpolationFilters="sRGB">
          ...
          <linearGradient id="flesh-root-tithe-target-arrow-gradient" gradientUnits="userSpaceOnUse" x1={start.x} y1={start.y} x2={end.x} y2={end.y}>
        ...
        <g filter="url(#flesh-root-tithe-target-arrow-glow)">
          <TacticalArrowGlyph path={arrow.path} tip={arrow.tip} color={ARROW_COLOR} stroke="url(#flesh-root-tithe-target-arrow-gradient)" />
        </g>
      </svg>
    </>
  );
