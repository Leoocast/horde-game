...
export function ZoneDrawer({ game }: { game: GameState }) {
  return (
    <section className="rounded-xl border border-white/15 bg-white/5">
      <div className="flex w-full items-center justify-between px-3 py-2 text-sm font-bold text-white">
...
function ZoneSide({ game, side }: { game: GameState; side: Side }) {
  ...
  return (
    <div>
      <h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-stone-300">{side}</h3>
...
function ZoneMetric({ label, count, top }: { label: string; count: number; top?: CardInstance }) {
  return (
    <div className="rounded-lg border border-white/10 bg-white/5 p-2">
      ...
      {top && <div className="mt-1 truncate text-[11px] text-stone-400">{top.displayName}</div>}
    </div>
  );
}
