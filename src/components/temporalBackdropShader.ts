...
void main() {
  ...
  color += vec3(0.72, 0.90, 1.0) * starLayer(near, 74.0, 0.970, 0.055) * 0.60;
  color += vec3(0.32, 0.72, 1.0) * starLayer(mid, 142.0, 0.992, 0.038) * 0.44;
  color += vec3(1.0, 0.66, 0.24) * starLayer(far, 97.0, 0.995, 0.026) * 0.42;

  float glow = fireflies(p + warp * 0.05, 9.0, 0.10, 0.0)
             + fireflies(p * 1.7 - warp * 0.04 + 8.2, 15.0, 0.07, 21.3) * 0.7;
  color += mix(vec3(1.0, 0.80, 0.36), vec3(0.55, 0.95, 0.78), 0.35) * glow * 0.85;
