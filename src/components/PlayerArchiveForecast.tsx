  ...
  return (
    <section
      ...
      aria-label={sourceDragActive
        ...
        : `${archiveLabel}: ${game.player.archive.length}. ${nextDrawLabel}: ${forecast.amount}.${emptyHandTooltip ? ` ${emptyHandTooltip}` : reasonLabel ? ` ${reasonLabel}.` : ""}`}
    >
      {sourceDragActive ? (
        <div className="source-return-target-box" aria-hidden="true">
          <span className="player-archive-emblem">
            <BookOpen size={26} strokeWidth={1.7} />
          </span>
          <span className="source-return-target-copy">
            <strong>{returnTitle}</strong>
            <span>{returnHint}</span>
          </span>
        </div>
      ) : (
        <>
