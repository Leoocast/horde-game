...
export function enqueue(game: GameState, event: Omit<EventItem, "id">): void {
  game.eventQueue.push({
    ...
    payload: {
      ...
      // an Echo that reaches the Field BECAUSE of this event reacts to it: Summoner of the Ranks
      // Invoking Rear-Guard Firebreather from the Archive made Rear-Guard Firebreather burn for the death that summoned it.
