...
example, four Corredores de Ascua y Chatarra, then Capataz del Recuento Ardiente, then two later tokens resolve in exactly
...
together. `tests/battlefieldLayout.test.js` covers the Alapútrida de la Cripta (`blighted_bat`) regression.
...
Use this contract for Artillero de los Últimos Remaches (`pashalik_mons`), Maestro de la Salva de
Escoria (`volley_veteran`), and future Goblin burn effects.

### Burn volley to player life

Lluvia de Remaches (`raid_bombardment`) reuses Burn with a different target and timing:
...
Varka, Eje de la Revuelta (`goblin_chainwhirler`) uses the same compact volley clock with distinct routes:
...
Mariscal del Golpe Repetido (`general_kreat_the_boltbringer`) does not aggregate its creature-entry triggers. Every other creature entering queues
...
Mariscal de la Última Marcha (`diregraf_captain`) preserves its printed life-loss semantics while using the Burn presentation:
...
Brakka or the Mariscal are covered too. Keyword grants are announced but never withheld: a keyword
...
Capataz del Recuento Ardiente's Burn.
...
- Side matters. The **left** of the Host panel is the Memory side — that is where its button lives — so a dying card is presented there (`host-death-reveal-host`) and exits into it. The **right** (`host-special-card-host`) stays reserved for spells and reveals still resolving, such as Diezmo de Carne y Raíz (`smallpox`).
